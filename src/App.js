...
class App extends Component {
  state = {
    ...
    timer: 60,
    tabsData: tabsList[0],
    thumbnaiInitailImage: imagesList[0],
    scoreCard: false,
  }

  componentDidMount() {
    console.log('Mount')
    this.update = setInterval(this.runTimer, 1000)
  }
  tabOnclick = uniqeId => {
    // console.log(uniqeId)
    const activeTab = tabsList.filter(each => {
      return each.tabId === uniqeId
    })
  ...
  runTimer = () => {
    ...
    this.setState(prev => {
      if (prev.timer === 0) {
        ...
        return {scoreCard: !prev.scoreCard}
      }
      return {
        timer: prev.timer - 1,
      }
  ...
  findUniqueImg = id => {
    ...
    const {thumbnaiInitailImage} = this.state

    const valueOfThumbnail = thumbnaiInitailImage.thumbnailUrl
    // console.log(valueOfThumbnail)
    //console.log(id)
    const matchGameVerify = id === valueOfThumbnail
    //  console.log(matchGameVerify)
    const randomNum = Math.floor(Math.random() * 30)
    if (matchGameVerify) {
      this.setState(prev => {
        return {
          score: prev.score + 1,
          thumbnaiInitailImage: imagesList[randomNum],
        }
      })
    } else {
      this.setState(prev => {
  ...
  displaySCoreCard = score => {
    return (
      <div className="displayWin">
        ...
        <p>Your Score</p>
        <p>{score}</p>
        <button type="button" className="btn" onClick={this.reStartGame}>
          <img
  ...
  render() {
    const {score, timer, tabsData, thumbnaiInitailImage, scoreCard} = this.state
    // console.log(scoreCard)
    const {tabId} = tabsData
    const filteteredData = imagesList.filter(each => {
      return each.category === tabsData.tabId
    })
    // console.log(filteteredData)
    const imgValue = thumbnaiInitailImage.imageUrl
    ...
    return (
      <div className="container">
        <div className="navBar">
          ...
          <ul className="scoreNav">
            <p>
              Score:<span className="scorePara">{score}</span>
            </p>
            <li className="clockInSec">
              <img
                className="logoTimer"
                src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
                alt="timer"
              />
              <p className="sec">{timer} sec</p>
            </li>
          </ul>
        </div>
        <div className="bottonSection">
          {scoreCard ? null : (
            <div className="findMatch">
              <img src={imgValue} className="macthImg" alt="match" />
            </div>
          )}
          {scoreCard ? null : (
          ...
          <div className="imgsList">
            {scoreCard ? (
              this.displaySCoreCard(score)
            ) : (
              <ul className="imgsCon">
                {filteteredData.map(each => {
                  return (
                    <TabItems
                      obj={each}
                      key={each.id}
                      findUniqueImg={this.findUniqueImg}
                    />
                  )
                })}
              </ul>
